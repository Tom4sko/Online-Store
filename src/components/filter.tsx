...
type ColorName = "Red" | "Yellow" | "Blue" | "Black" | "White";
const categories = ["t-shirts", "caps", "shoes", "gloves", "jackets"];
...
const filter = ({ selectedSizes, selectedColors, selectedCategories, onSizeChange, onColorChange, onCategoryChange }: FilterProps) => {
    ...
    const colors: ColorName[] = ["Red", "Yellow", "Blue", "Black", "White"];
      ...
      <div className="w-full flex flex-col overflow-y-scroll max-h-[calc(100vh-300px)] md:max-h-max">
        ...
        <div className="flex flex-col md:flex-row items-start gap-2 pb-5 pl-1">
          {colors.map(color => (
            <span 
              key={color} 
              className={`block size-6 ${colorMap[color]} cursor-pointer ${selectedColors.includes(color) ? 'ring-2 ring-offset-2 ring-black' : ''}`}
